...
const config = {
  ...
  beforePack: async (context) => {
    ...
    if (
      context.appOutDir.endsWith("mac-universal--x64") ||
      context.appOutDir.endsWith("mac-universal--arm64")
    ) {
    ...
    if (
      ...
      context.packager.config.extraResources.push(
        "extensions/podman/assets/podman-installer-macos-aarch64-*.pkg"
      );
    }
    ...
    if (
      ...
      context.packager.config.extraResources.push(
        "extensions/podman/assets/podman-installer-macos-amd64-*.pkg"
      );
    }
    ...
    if (context.electronPlatformName === "win32") {
      ...
      context.packager.config.extraResources.push(
        "extensions/podman/assets/podman-*.exe"
      );
    }
    if (context.arch === Arch.x64 && context.electronPlatformName === "win32") {
      context.packager.config.extraResources.push(
        "extensions/podman/assets/podman-image-x64.tar.xz"
      );
    }
    if (
      context.arch === Arch.arm64 &&
      ...
      context.packager.config.extraResources.push(
        "extensions/podman/assets/podman-image-arm64.tar.xz"
      );
    }
  },
  ...
  flatpak: {
    ...
    files: [
      [
        ".flatpak-appdata.xml",
        "/share/metainfo/io.podman_desktop.PodmanDesktop.metainfo.xml",
      ],
      [
        "buildResources/icon-512x512.png",
        "/share/icons/hicolor/512x512/apps/io.podman_desktop.PodmanDesktop.png",
      ],
    ],
  },
  linux: {
...
const azureCodeSign = (filePath) => {
  if (!process.env.AZURE_KEY_VAULT_URL) {
    console.log(
      "Skipping code signing, no environment variables set for that."
    );
